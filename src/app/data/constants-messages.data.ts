export const API_URL = {
    CSGO_API: "/api/GetItemsList/v2/",
    CSGO_API_IMAGE: "http://cdn.steamcommunity.com/economy/image/",
    REGISTER: "api/register",
    LOGIN: "api/login",
    GET_TRY_OUT_ITEMS: "api/getTryOutItems",
    GET_MARKET_ITEMS: "api/getMarketItems",
    GET_OWNED_ITEMS: "api/getOwnedItems",
    BUY_ITEM: "api/buyItem",
    SELL_ITEM: "api/sellItem",
    OPEN_CONTAINER: "api/openContainer",
    OPEN_TRY_OUT_CONTAINER: "api/openTryOutContainer",
    GET_ITEM_IMAGE: "steamApi/",
    GET_AUTOCOMPLETE_OPTIONS: "api/getAutocompleteOptions?property=",
    SET_USER_SETTINGS: "api/setUserSettings",
    GET_LAST_OPENED_ITEMS: "api/getLastOpenedItems"
};

export const DEFAULT_CURRENCY = "USD";
export const DEFAULT_LANGUAGE = "en";
export const DEFAULT_DARK_THEME = true;
export const DEFAULT_BACKGROUND_COLOR_DIRECTIVE = "rgb(176, 195, 217)";
export const DEFAULT_TEXT_COLOR_DIRECTIVE = "rgb(0, 226, 0)";
export const PRICE_HIGHEST = "ITEM_LIST_FILTERS.PRICE_HIGHEST";
export const PRICE_LOWEST = "ITEM_LIST_FILTERS.PRICE_LOWEST";
export const SORT_ASCENDING = "asc";
export const SORT_DESCENDING = "desc";
export const REGISTER_LABEL = "REGISTER";
export const LOGIN_LABEL = "LOGIN";
export const ACCESS_TOKEN_STORAGE_KEY = "accessTokenId";
export const ACCESS_TOKEN_EXPIRES_AT_STORAGE_KEY = "accessTokenExpiresAt";
export const USER_DATA_STORAGE_KEY = "userData";
export const SUCCESS = "Success";
export const FAIL = "Fail";
export const MARKET_URL = "market";
export const EQUIPMENT_URL = "equipment";
export const TRY_OUT_URL = "try-out";
export const OPEN_CONTAINER_DIALOG_PANEL_CLASS = "drawnItemDialogContainer";
export const SUCCESS_TITLE = "LOGIN_DIALOG.SUCCESS_TITLE";
export const REGISTER_SUCCESS = "LOGIN_DIALOG.REGISTER_SUCCESS";
export const LOGIN_SUCCESS = "LOGIN_DIALOG.LOGIN_SUCCESS";
export const NO_IMAGE_URL = "./assets/images/no-image.png";
export const GET_LAST_OPENED_ITEMS_INTERVAL = 3000;
